...
const { Circuit, Driver, Event, Result, Season, Team } = require('../models');
const {
  circuitsData,
  driversData,
  eventsBySeason,
    ...
    mongoose
      ...
      .then(() => {
        const circuitPromises = Object.keys(circuitsData).map((circuit) =>
          new Circuit(circuitsData[circuit]).save().then((doc) => {
        ...
        const teamsPromises = Object.keys(teamsData).map((team) =>
          new Team(teamsData[team]).save().then((doc) => {
            teamIds[team] = doc.id;
            return doc;
          })
        );
        ...
        const driverPromises = Object.keys(driversData).map((driver) =>
          new Driver(driversData[driver]).save().then((doc) => {
        ...
        const seasonPromises = Object.keys(seasonsData).map((season) =>
          new Season(seasonsData[season]).save().then((doc) => {
        ...
        return Promise.all([
          ...
          .then((data) => {
            ...
            Object.keys(eventsBySeason).forEach((seasonYear) => {
              let year = eventsBySeason[seasonYear].map((event) => {
                let date =
                  ...
                  new Date(event.date + 'T12:00').getTime();
                return new Event({
            ...
            return Promise.all([Promise.all(eventsPromises), ...data]);
          })
          .then((data) => {
            let resultsPromises = [];
            Object.keys(resultsBySeason).forEach((year) => {
              Object.keys(resultsBySeason[year]).forEach((round) => {
                let roundResults = Object.keys(
                  resultsBySeason[year][round]
                ).map((position) => {
                  const {
                    driver,
                    constructor,
            ...
            return Promise.all([...resultsPromises, ...data]);
          })
          .then((data) => {
